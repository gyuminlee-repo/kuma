...
이 아카이브는 v0.13.30 에서 멈춥니다. v0.14 이후 릴리스는 여기가 아니라 [CHANGELOG.md](CHANGELOG.md) 에 기록됩니다. 아래 항목은 각각 자기가 속한 릴리스 시점을 적은 것이며 현재 동작이 아닙니다.
...
> 대체됨. 여기 적힌 RPC와 store 액션은 v0.3 xlsx 파이프라인이 자리를 넘겨받은 뒤 제거했다. 그 시점에 이미 호출하는 UI 가 없었다. `export_evolvepro_csv` 함수 자체는 `kuma_core/mame/activity/export_evolvepro.py` 에 남아 있다. `tests/integration/test_kuma_round_trip.py` 가 MAME writer 와 KURO 리더의 컬럼 합의를 고정하는 데 쓰기 때문이다. 현재 동작은 `docs/mame/04-activity.md` 를 본다.
