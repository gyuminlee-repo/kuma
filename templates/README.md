...
- **선정된 복제본(FINAL) 은 전부 PASS 다 (2026-08-31 재작업).** PASS 아닌 verdict 는 대표로 선출하지 않는다는 사용자 규칙에 따른 것이다. `kuma_core/mame/select/best_pick.py` 의 `PRIORITY_ORDER`(`PASS > AMBIGUOUS > LOWDEPTH`)가 대표를 뽑는 층위이고, WRONG_AA·MANY·FRAMESHIFT·MIXED·NO_CALL 은 그 목록에 없어 세 복제본이 전부 그 클래스면 `pick_best_replicate` 는 대표를 못 뽑고 `is_fallback=True` 인 폴백으로 넘어간다(2026-08-31 이전 상태가 이것이었다: 8종 중 6종이 FINAL 에서 비-PASS·fallback 이었다). 그래서 8종 중 AMBIGUOUS·LOWDEPTH·WRONG_AA·MANY·FRAMESHIFT·MIXED·NO_CALL 7종은 각 변이(`K239A`·`G190A`·`H200A`·`K215A`·`Q205A`·`K210A`·`A227V`·`D235A`, 이 중 LOWDEPTH 는 두 변이에서 재현)의 native barcode 3개 중 **정확히 하나에만** 그 결함을 심고, 나머지 둘은 정상 치환만 넣어 PASS 로 읽히게 한다. 결함을 심는 barcode 는 변이마다 NB01/NB02/NB03 에 분산돼 있어 한 barcode 가 "불량 로드"로 고정되지 않는다. 대표는 나머지 두 PASS 복제본 중 Wilson 하한이 높은 쪽이 되므로 8종 전부 FINAL=PASS 다(16 변이 + WT = 17건 전부 PASS, `is_fallback` 0건). 8종의 verdict 다양성은 사라지지 않고 전체 51건 verdict 안에서 각 결함 well 로 그대로 남으며, 복제본 비교 화면에서만 보인다. 전부 consensus 서열과 FASTA 헤더 값만으로 유발하므로 `classify_verdict` 와 `best_pick.py` 는 손대지 않았고 픽스처도 손으로 고치지 않았다.
