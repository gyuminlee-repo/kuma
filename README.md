...
- **EVOLVEpro CSV 입력**: `df_test.csv` 로드 → y_pred 내림차순 정렬 → 상위 96개 자동 선정
...
- **전체 프라이머 Tm 타겟**: Fwd 62°C, Rev 58°C, Overlap 42°C (SantaLucia 1998, Benchling 일치 파라미터)
- **점진적 Tm tolerance**: ±0.5°C부터 시작, 후보 없으면 ±0.5씩 확장 (최대 ±3.0°C, 변이별 독립)
- **GC% penalty**: 40-60% 범위 밖 프라이머에 패널티 부여, 35% 미만/65% 초과 시 경고
- **후보 비교 및 교체**: 각 변이에 대한 모든 프라이머 후보를 비교하고, 수치 비교 후 교체 가능
- **Off-target 검증**: template sense/antisense strand에서 비특이적 결합 자동 검출
- **폴리머라제 프로필**: 자체 내장 6종 프로필 (Benchling, Taq, Phusion, Q5, KOD, DreamTaq)
- **프라이머 리스트**: Fwd/Rev 쌍 순차 나열 + Excel 출력
...
1. 시퀀스 파일 로드 (FASTA / SnapGene .dna)
2. ATG 드롭다운에서 타겟 유전자 CDS 확인 (자동 선택)
3. 변이 입력 (텍스트 직접 입력 또는 EVOLVEpro CSV 로드)
4. 폴리머라제 선택 (기본: KOD)
5. Design Primers 클릭
6. Fwd/Rev 서열 클릭 → 후보 비교 팝오버에서 교체 가능
7. File → Export TSV / Export Excel
...
├── python-core/                  사이드카 래퍼
│   ├── sidecar_main.py           JSON-RPC 디스패처 (9개 메서드)
│   └── build_sidecar.py          PyInstaller 빌드 스크립트
├── evolveprimer/                 Python 백엔드
│   ├── sdm_engine.py             SDM 설계 엔진 (upstream overlap + 전체 Tm + off-target)
