...
- **주황 배지** = 엔진이 해당 프라이머 쌍의 구조를 경고로 판정. hairpin은 쌍의 권장 어닐링 온도에서 이상태(two-state) folded fraction이 10%를 넘으면 경고(프로파일에 Ta 규칙이 없으면 60 °C fallback), homodimer는 Tm이 40 °C를 넘으면 경고. 같은 판정이 세부 popover의 상태 컬럼에도 적용된다.
