...
프로젝트를 새로 만들면 spotlight 투어가 공통 프로젝트 표시줄을 소개한 뒤 Kuro의 워크플로 레일, 주 작업 영역 및 인스펙터를 강조한다. **다음**으로 계속하고, **모든 투어 건너뛰기**로 해당 프로젝트의 모든 투어를 비활성화하거나, `Esc`로 현재 투어만 닫을 수 있다. 이후 **도움말 → 가이드 투어 보기**에서 다시 실행할 수 있다. 기존 프로젝트에는 자동 표시되지 않는다.
