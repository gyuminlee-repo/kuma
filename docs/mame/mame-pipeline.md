...
| indel event gate 0.50 | **자체 실측** 위에 놓은 값 (이 저장소에서 재실행 불가) | 운용 임계값은 `max_indel_event_fraction` 기본값 0.50 이고 비교는 `>=`. 근거는 `bench_v2 depth_50` 관측값으로 노이즈 웰 <= 0.21, 진짜 결손 웰 >= 0.83. bench_v2 의 데이터와 실행 조건은 이 저장소에 없다 |
