...
Tests:
  - test_get_egfp_wt_aa_seq_returns_string: result is a non-empty A-Z string
...
from kuma_core.mame.activity.ref_seq import DEFAULT_EGFP_CDS_PATH, get_egfp_wt_aa_seq
...
class TestGetIspSWtAaSeqReturnsString:
    ...
    def test_default_cds_path_exists(self):
        assert DEFAULT_EGFP_CDS_PATH.exists(), (
            f"DEFAULT_EGFP_CDS_PATH not found: {DEFAULT_EGFP_CDS_PATH}"
        )
...
class TestCachingReturnsSameObject:
    ...
    def test_two_calls_return_same_object(self):
        aa1 = get_egfp_wt_aa_seq()
        aa2 = get_egfp_wt_aa_seq()
        assert aa1 is aa2
...
class TestCustomPathOverride:
    ...
    def test_minimal_cds_translates_correctly(self, tmp_path):
        ...
        aa = get_egfp_wt_aa_seq(cds_path=fa)
        assert aa == "MK"
    ...
    def test_different_path_gives_different_result(self, tmp_path):
        ...
        aa_custom = get_egfp_wt_aa_seq(cds_path=fa)
        aa_default = get_egfp_wt_aa_seq(cds_path=None)
...
class TestMissingFileRaises:
    ...
    def test_empty_fasta_raises_value_error(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="empty protein sequence"):
            get_egfp_wt_aa_seq(cds_path=fa)
