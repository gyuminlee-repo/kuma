...
KURO is part of kuma, a Tauri v2 desktop app with a thin Rust shell, a React 19 frontend, and KURO/MAME Python sidecars. The Rust layer in `src-tauri/` owns the desktop host, windowing, bundling, and sidecar lifecycle and request routing. The frontend in `src/` owns the application state and UI. The scientific logic does not live in Rust.

The frontend clients (`src/lib/ipc.ts` and `src/lib/ipc-mame/`) invoke Tauri commands. `src-tauri/src/sidecar.rs` manages the processes and routes JSON-RPC over stdin/stdout. The dispatchers are `python-core/sidecar_kuro/dispatcher.py` and `python-core/sidecar_mame/dispatcher.py`; each routes methods to its own handlers and validates requests with its own Pydantic models.

The primer-design and EVOLVEpro logic lives in `kuma_core/kuro/`, and NGS verification lives in `kuma_core/mame/`. Keep these Python domain packages independent of Tauri or frontend concerns. When KURO request or response shapes change, update both `python-core/sidecar_kuro/models.py` and `src/types/models.ts` together.
...
- `src/types/models.ts` and `python-core/sidecar_kuro/models.py` must stay in sync. This is especially important for workspace payloads, excluded ranges, rescue stats, and any JSON-RPC request/response fields.
- If you change `kuma_core/kuro/evolvepro.py` selection columns or validation-related fields, also check fixtures and any sample-data generation paths that depend on them.
