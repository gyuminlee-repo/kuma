...
function extractFiles(payload) {
  ...
  if (Array.isArray(ti.edits)) {
    return ti.edits.map((e) => e?.file_path).filter(Boolean);
  }
  return [];
}
...
function matchGroups(file, groups) {
  return groups.filter((g) =>
    (g.files ?? []).some((pat) => picomatch(pat, { dot: true })(file))
  );
}
...
function formatOutput(file, groups) {
  ...
  const overflow = groups.length - cap;
  const lines = [`[kuma-deps] ${file} 변경 감지`, ""];
...
async function main() {
  ...
  try {
    ...
    process.stderr.write(`[kuma-deps] config parse error: ${e.message}\n`);
    process.exit(0);
  }
...
main().catch((e) => {
  process.stderr.write(`[kuma-deps] unexpected error: ${e.message}\n`);
  process.exit(0);
});
