...
// ---------------------------------------------------------------------------
// EvolveProSlice (Wave 1b independent module, no other-slice dependencies)
// ---------------------------------------------------------------------------
...
export interface EvolveProEnvStatus {
  env_found: boolean;
...
export interface EvolveProRunRequestParams {
  input_csv: string;
  wt_sequence: string;
  n_rounds: number;
  output_dir: string;
...
export interface EvolveProProgress {
  run_id: string;
  stage: "detect" | "loading" | "scoring" | "selecting" | "done";
  current: number;
  total: number;
  message: string;
}

export interface EvolveProResult {
  run_id: string;
  output_csv: string;
  top_variants: string[];
  elapsed_sec: number;
...
export interface EvolveProSlice {
  // State
  evolveProEnvStatus: EvolveProEnvStatus | null;
  evolveProRunId: string | null;
  evolveProProgress: EvolveProProgress | null;
  evolveProResult: EvolveProResult | null;
  ...
  startEvolveProRun: (request: EvolveProRunRequestParams) => Promise<void>;
  cancelEvolveProRun: () => Promise<void>;
  setEvolveProProgress: (p: EvolveProProgress) => void;
  resetEvolveProState: () => void;
}
