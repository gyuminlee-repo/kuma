...
2. MAPQ >= 25 filter.
3. 100% coverage filter: r_st == 0 and r_en == ref_len.
4. For each alignment, determine strand-normalised sequence and anchor coords:
   - strand +1: use read_seq as-is.  anchor = (q_st, q_en).
   - strand -1: normalise to RC(read_seq), remap coords to
                (L - q_en, L - q_st) so the same window logic applies.
5. Barcode demux using edlib HW (infix) edit-distance search:
...
def run_combinatorial_demux(
    ...
    """MAPQ-filtered alignment-anchored fuzzy per-well demux.

    Aligns pooled reads to a single reference, applies 100% reference coverage
    filter, assigns each read to an R×F well by alignment-anchored edlib fuzzy
    barcode matching, and calls majority-vote consensus per well.
    ...
    trim_flank_bp:
        Bases to include on each side of aligned region when writing FASTA
        (default 30; not used for barcode windows which use window_bp).
    min_depth:
    ...
    log.info(
        "Barcode-assigned reads: %d  (ambiguous/no-match dropped: %d)",
