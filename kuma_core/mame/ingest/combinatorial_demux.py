...
- The tail is DERIVED from the file: the longest common suffix of the F column,
  and of the R column, computed by ``barcode_tail.common_tail``. The ispS file
  yields exactly the historical constants (F 16 bp ``cacaggaggttaaacc``, R 15 bp
  ``tgcgttgcgctctag``), and a package generated by ``barcode_package`` yields
  whatever flanking primer was designed for that gene, which is the case a
  constant could never cover.
- There is NO fallback. A workbook whose axes state no shared tail is refused
  with ``ValueError``. The rejected paths used to be a search for the ispS
  constants and then a fixed 11 bp (F) / 10 bp (R) cut; both return a seed of
  the wrong length as soon as the campaign is not ispS, the reverse axis is the
  plate row, and a mis-cut reverse seed files reads under a row that was never
  pipetted while every other number in the result looks fine. A refusal an
  operator can act on beats a run nobody can audit.
- A KNOWN CONSEQUENCE of deriving the tail: an axis carrying a single row is
  refused, because one sequence shares nothing with anything and any cut on it
  would be a length nobody wrote down. This is not reachable from a file kuma
  wrote (``barcode_package`` designs 12 forward and 8 reverse, and
  ``barcode_package.parse_barcode_seeds`` refuses any other count), so it can
  only arrive as a hand-trimmed workbook, which is exactly the file the old
  fixed-length cut used to guess at.
...
from kuma_core.mame.ingest.barcode_tail import (
    MIN_SEED_LENGTH,
    MIN_TAIL_LENGTH,
...
# NOTE (2026-08-06): the ispS annealing constants ('cacaggaggttaaacc' /
# 'tgcgttgcgctctag') and the fixed 11 bp / 10 bp fallback lengths used to live
# here, together with _extract_barcode_prefix / _extract_f_prefix /
# _extract_r_prefix. All of them are deleted, not deprecated. They described one
# campaign, and every path that reached them returned a seed whose length
# nothing in the workbook stated. The tail is now derived from the file or the
# file is refused; see _resolve_axis_prefixes. The literals survive only as
# fixture data inside tests and python-core/scripts/bench_demux_match.py, which
# synthesise ispS-shaped reads, and that is the only place a campaign constant
# belongs.
...
class BarcodeAxisPrefixes:
    """The seeds read off one axis, and the tail that was cut to get them.

    There is no ``method`` field and no ``guessed`` flag any more. Both existed
    to name a fallback, and an axis that needs one is now refused before this
    object is built, so every instance describes the same single path: a tail
    the file itself stated. A field whose value is fixed carries no information
    and invites a reader to believe some other value is reachable.
    """

    #: ``"F"`` or ``"R"``.
    axis: str
    #: The tail that was stripped, uppercase, at least ``MIN_TAIL_LENGTH`` long.
    tail: str
    #: ``(name, seed)`` in barcode-index order, exactly what the matcher uses.
    barcodes: tuple[tuple[str, str], ...] = ()

    @property
    def seed_lengths(self) -> tuple[int, ...]:
        """Length of each seed, in the same order as :attr:`barcodes`.

        Reported because it is the number an operator can check against the
        seed workbook they ordered primers from. A cut that ran into the seeds
        shortens every one of them by the same amount, so a distribution that
        does not match the order is the visible symptom of that.
        """
        return tuple(len(seed) for _, seed in self.barcodes)

    def as_dict(self) -> dict:
        """This axis as the sidecar puts it on the analyze response."""
        return {
            "axis": self.axis,
            "tail": self.tail,
...
@dataclass(frozen=True, slots=True)
class BarcodePrefixResolution:
    """Both axes of a barcode workbook, and what was derived from each.

    Every instance describes a file that explained itself; a file that does not
    raises out of :func:`load_barcode_prefixes_with_provenance` instead of
    reaching this type. What it carries is therefore not "which of three paths
    was taken" but "what was cut, and how much seed is left", which is the pair
    an operator can check against the seed workbook the primers were ordered
    from.
    """
    ...
    @property
    def note(self) -> str:
        """One operator-facing paragraph about what this file stated."""
        return (
            "Barcode seeds were cut at the sequence every primer on an axis "
            "shares to its 3' end, read off this file: "
            f"{len(self.forward.tail)} bp forward ({self.forward.tail}) leaving "
            f"{_seed_length_span(self.forward)} of seed, "
            f"{len(self.reverse.tail)} bp reverse ({self.reverse.tail}) leaving "
            f"{_seed_length_span(self.reverse)}. That shared sequence is the "
            "annealing region unless the seeds themselves also end alike, in "
            "which case the cut runs a base or two into them, evenly across the "
            "axis, and the seeds stay mutually distinguishable. No length was "
            "assumed anywhere: a workbook whose axes share no such sequence is "
            "refused rather than read at a guessed seed length."
        )

    def as_dict(self) -> dict:
        """The payload the sidecar puts on its analyze response."""
        return {
            "forward": self.forward.as_dict(),
            "reverse": self.reverse.as_dict(),
            "note": self.note,
        }


def _seed_length_span(axis: BarcodeAxisPrefixes) -> str:
    """``"11 bp"`` for one length, ``"10-11 bp"`` when the axis is ragged."""
    lengths = axis.seed_lengths
    if not lengths:
        return "no seeds"
    low, high = min(lengths), max(lengths)
    return f"{low} bp" if low == high else f"{low}-{high} bp"


#: Long name of each axis, for messages an operator reads.
_AXIS_WORD = {"F": "forward", "R": "reverse"}


def _axis_refusal(axis: str, source: Path, reason: str, remedy: str) -> str:
    """One refusal sentence: which file, which axis, why, and what to do."""
    return (
        f"custom_barcodes_xlsx: {Path(source).name} does not state where its "
        f"{_AXIS_WORD.get(axis, axis)} barcode seeds end. {reason} {remedy}"
    )


#: What to do about every refusal below. Named once because the answer is the
#: same one whichever way the axis failed: the workbook has to carry one shared
#: flanking primer per axis, and kuma writes exactly that shape.
_REEXPORT_REMEDY = (
    "Every primer on one axis has to end with the same flanking primer, because "
    "that shared sequence is the only thing in the file that marks where the "
    "seed stops; without it the seed length is a guess, and the reverse axis is "
    "the plate row, so a wrong guess files reads under a row that was never "
    "pipetted. Re-export the workbook from the MAME barcode package generator "
    "(kuma_core.mame.ingest.barcode_package), which writes seed + shared flank "
    "on every row."
)

#: What to do when a minority of named rows are the whole reason the axis has
#: no shared tail. Separate from ``_REEXPORT_REMEDY`` because the two are
#: different actions and only one of them is right here: the file already has
#: the shape kuma writes, so re-exporting it would replace seeds the primers
#: were ordered against in order to fix a cell.
_ODD_ROW_REMEDY = (
    "The rest of the axis does share a tail, so this is the right file shape "
    "with a damaged cell rather than the wrong file shape: open column B of "
    "the rows named above and look for a truncated paste, a stray character or "
    "a flanking primer from another design, and correct those cells. Only "
    "re-export the workbook from the MAME barcode package generator "
    "(kuma_core.mame.ingest.barcode_package) if the design itself is wrong, "
    "because a re-export issues new seeds."
)
...
def _odd_rows_out(
    ...
    """Which rows break an otherwise shared 3' end, and what the rest share.

    Returns ``(row names, bases those rows share)``, or ``((), 0)`` when no
    strict majority of the axis ends the same way. The majority test is the
    whole point: one damaged cell among twelve is a cell to fix and can be
    named, whereas an axis whose rows disagree in halves has no reference to be
    odd against and naming either half would be picking a side.

    Rows are compared on their last ``MIN_TAIL_LENGTH`` bases, which is the
    shortest 3' end this module is willing to call an annealing region, so a
    row is called odd on exactly the span the acceptance test would have used.
    """
    if len(entries) < 3:
        return (), 0
    probe = Counter(seq[-MIN_TAIL_LENGTH:] for _, _, seq in entries)
    majority, count = probe.most_common(1)[0]
    if count * 2 <= len(entries):
        return (), 0
    agreeing = [seq for _, _, seq in entries if seq[-MIN_TAIL_LENGTH:] == majority]
    odd = tuple(
        name for _, name, seq in entries if seq[-MIN_TAIL_LENGTH:] != majority
    )
    return odd, common_suffix_length(agreeing)


def _resolve_axis_prefixes(
    axis: str,
    entries: list[tuple[int, str, str]],
    source: Path,
) -> BarcodeAxisPrefixes:
    """Strip the annealing tail off one axis, or refuse the workbook.

    ``entries`` are ``(index, name, uppercase sequence)``; they are sorted by
    index here so the returned order is the plate order the matcher assumes.

    The tail is the longest common suffix of the axis, and there is no second
    way to get one. Two fallbacks used to sit behind this: a search for the ispS
    annealing constants, and a fixed 11 bp / 10 bp cut. Both produced a seed
    whose length nothing in the workbook stated, the run then finished reporting
    plausible wells, and the mis-cut was invisible in every cell of the result.
    Labelling that outcome was not enough, so it is refused.

    Raises
    ------
    ValueError
        When the axis has no rows, has one row, shares fewer than
        ``MIN_TAIL_LENGTH`` bases to its 3' end, or shares so many that fewer
        than ``MIN_SEED_LENGTH`` bases of seed would be left.
    """
    ...
    if not ordered:
        raise ValueError(
            _axis_refusal(
                axis,
                source,
                f"It carries no {_AXIS_WORD.get(axis, axis)} barcode rows at all "
                "(rows are named <prefix>_f_<n> and <prefix>_r_<n>, for example "
                "egfp_f_1 and egfp_r_1).",
                _REEXPORT_REMEDY,
            )
        )

    sequences = [seq for _, _, seq in ordered]
    derived = common_tail(sequences)
    if derived is not None:
        cut = len(derived)
        return BarcodeAxisPrefixes(
            axis=axis,
            tail=derived,
            barcodes=tuple((name, seq[: len(seq) - cut]) for _, name, seq in ordered),
        )

    # Refused. Say how far short the file fell, and where, because "fix these
    # three cells", "re-export it" and "shorten the seeds" are different actions
    # and the operator picks between them on what this sentence says.
    remedy = _REEXPORT_REMEDY
    if len(sequences) == 1:
        reason = (
            f"Read 1 {_AXIS_WORD.get(axis, axis)} row, and where the seed ends "
            "is something an axis states by repetition: with one row there is "
            "nothing to compare against, so any cut would be a length nobody "
            "wrote down. An axis needs at least two rows to state its own rule, "
            "and a MAME plate carries 12 forward and 8 reverse."
        )
    else:
        shared = common_suffix_length(sequences)
        if shared < MIN_TAIL_LENGTH:
            reason = (
                f"Read {len(sequences)} {_AXIS_WORD.get(axis, axis)} rows of "
                f"{min(map(len, sequences))}-{max(map(len, sequences))} bp; the "
                f"sequence all of them share to their 3' end is {shared} bp, "
                f"short of the {MIN_TAIL_LENGTH} bp an annealing region has to "
                "reach before it can be told from a coincidence."
            )
            # Name the rows when a majority of the axis does agree, because
            # then the file is not the wrong shape and "re-export it" is the
            # wrong instruction: what stands between this run and a correct
            # one is a handful of cells the operator can see.
            odd, agreed = _odd_rows_out(ordered)
            if odd:
                reason += (
                    f" The other {len(sequences) - len(odd)} rows do share "
                    f"{agreed} bp to their 3' end; the ones that end "
                    f"differently are {', '.join(odd)}."
                )
                remedy = _ODD_ROW_REMEDY
        else:
            shortest_seed = min(len(seq) - shared for seq in sequences)
            reason = (
                f"Read {len(sequences)} {_AXIS_WORD.get(axis, axis)} rows sharing "
                f"{shared} bp to their 3' end, which leaves {shortest_seed} bp of "
                f"seed in front of it, under the {MIN_SEED_LENGTH} bp minimum. "
                "Either the primers were designed under a shared 3' constraint, "
                "so the common suffix runs past the annealing region into the "
                "seeds, or the seeds are too short to tell wells apart."
            )
    raise ValueError(_axis_refusal(axis, source, reason, remedy))


def load_barcode_prefixes_with_provenance(
    barcodes_xlsx: Path,
) -> BarcodePrefixResolution:
    """Load both barcode axes and report how each one was read.

    This is the full reader; :func:`load_barcode_prefixes` is the two-list view
    of the same work and is what the pipeline calls. The split is deliberate:
    ``load_barcode_prefixes`` is called from demux workers, from
    ``run_combinatorial_demux`` and from tests, all of which unpack exactly two
    lists, and widening that return type would have touched every one of them to
    carry a field none of them reads. An out-parameter was the other candidate
    and was rejected for the same reason it usually is, an argument that only
    exists to be written to. So the provenance lives on a separate entry point,
    the sidecar analyze handler calls that one, and the value rides to the
    frontend on the analyze response (``barcode_prefix_resolution``) where an
    operator can see what was cut off their primers.

    Raises
    ------
    ValueError
        When either axis does not state where its seeds end. Both entry points
        raise: refusing on one and guessing on the other would put the decision
        in the hands of whichever caller a run happened to go through.
    """
    try:
    ...
    # Full sequences here, not prefixes: the tail cannot be derived one row at a
    # time, it is a property of the whole axis.
    f_entries: list[tuple[int, str, str]] = []  # (idx, name, sequence)
    ...
    for row in ws.iter_rows(values_only=True):
        ...
        if m_f is not None:
            f_entries.append((int(m_f.group("n")), name, seq_val.upper()))

        elif m_r is not None:
            r_entries.append((int(m_r.group("n")), name, seq_val.upper()))

    wb.close()

    # Either axis may refuse the file; neither returns a guessed seed.
    resolution = BarcodePrefixResolution(
        forward=_resolve_axis_prefixes("F", f_entries, barcodes_xlsx),
        reverse=_resolve_axis_prefixes("R", r_entries, barcodes_xlsx),
    )

    for reading in (resolution.forward, resolution.reverse):
        log.debug(
            "%s barcode seeds cut at a %d bp tail derived from %s.",
            reading.axis,
            len(reading.tail),
            barcodes_xlsx,
        )
...
def load_barcode_prefixes(
    ...
    The prefix is the barcode-unique region only (tail excluded).  Its length is
    whatever the file implies: 10 bp (R and most F) or 11 bp (F1-F3) in the ispS
    plate, and the designed seed length in a workbook ``barcode_package``
    generated. Callers that want the tail that was cut and the seed lengths it
    left call :func:`load_barcode_prefixes_with_provenance` instead; this
    signature is unchanged so demux workers and tests keep working.

    Raises
    ------
    ValueError
        When either axis does not state where its seeds end; see
        :func:`_resolve_axis_prefixes`.
    """
    resolution = load_barcode_prefixes_with_provenance(barcodes_xlsx)
    return list(resolution.reverse.barcodes), list(resolution.forward.barcodes)
