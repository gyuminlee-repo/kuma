...
def _render_summary_cards(data: RunReportData) -> str:
    ...
    # dropped, because the per-plate detected chips below use the same
    # PASS+AMBIGUOUS predicate and would otherwise have no run-level
    # counterpart. The chip counts and this card are not summands of each other:
    # a chip counts wells on one plate and this card counts designed mutants, so
    # a mutant detected on three plates is 3 in the chip totals and 1 here.
