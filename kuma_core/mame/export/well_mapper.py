...
The plate shape comes from :mod:`kuma_core.mame.plate_geometry`, which is also
what validates that a barcode file describes this plate before a run starts.
"""

from __future__ import annotations

from kuma_core.mame.plate_geometry import PLATE_CAPACITY, PLATE_COLS, PLATE_ROWS


def seq_to_well(seq: int) -> str:
    """Convert a 1-based sequence index (1..96) to a column-major well label."""

    if not 1 <= seq <= PLATE_CAPACITY:
        raise ValueError(f"seq must be in [1, {PLATE_CAPACITY}]; got {seq}")
    row_idx = (seq - 1) % PLATE_ROWS
    col_num = (seq - 1) // PLATE_ROWS + 1
