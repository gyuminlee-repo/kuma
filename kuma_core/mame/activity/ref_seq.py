"""IspS WT reference protein sequence loader.

OQ-④ decision: fixtures/ispS.fa CDS → BioPython translate → cached.
...
# Module-level constant — tests may monkey-patch DEFAULT_ISPS_CDS_PATH.
DEFAULT_ISPS_CDS_PATH: Path = (
    Path(__file__).resolve().parents[3] / "fixtures" / "ispS.fa"
)


@lru_cache(maxsize=4)
def get_isps_wt_aa_seq(cds_path: Path | None = None) -> str:
    """Return IspS WT protein 1-letter AA sequence (cached).
    ...
    Args:
        cds_path: Override path to a CDS FASTA file (for testing).
                  ``None`` uses ``DEFAULT_ISPS_CDS_PATH``.
    ...
    resolved: Path = cds_path if cds_path is not None else DEFAULT_ISPS_CDS_PATH

    if not resolved.exists():
        raise FileNotFoundError(
            f"IspS CDS FASTA not found: {resolved}"
        )

    records = list(SeqIO.parse(str(resolved), "fasta"))
    if not records:
        raise ValueError(
            f"IspS CDS FASTA contains no sequence records: {resolved}"
        )
